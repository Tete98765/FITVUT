...
void bst_delete(bst_node_t **tree, char key) {
  ...
  if((*tree)->key != key) { //nie je co mazat
    if (key < (*tree) -> key) bst_delete(&((*tree)->left), key);
	  else if (key > (*tree) -> key) bst_delete(&((*tree)->right), key);
  }
	else {	//naslo sa nieco na mazanie
		//nema synov
		if ((*tree)->left == NULL && (*tree)->right == NULL) {
			free ((*tree));
			(*tree) = NULL;
		}
    //ma len praveho syna, na jeho misto vlozi novu adresu syna
		else if ((*tree)->left == NULL && (*tree)->right != NULL) {
      bst_node_t *delete = (*tree);
		  (*tree) = (*tree) -> right;
			free (delete);
		}
		//ma len laveho syna, na jeho misto vlozi novu adresu syna
		else if ((*tree)->left != NULL && (*tree)->right == NULL) {
      bst_node_t *delete = (*tree);
			(*tree) = (*tree)->left;
			free(delete);
		...
		//ma oba syny
			...
			if ((*tree)->left->right != NULL) bst_replace_by_rightmost((*tree), &((*tree)->left->right));
